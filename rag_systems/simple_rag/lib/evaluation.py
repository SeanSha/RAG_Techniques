"""
RAG 评估：基于检索上下文对问题打分（relevance / completeness / conciseness），并生成 RAG 回答。
不依赖项目根目录的 evaluation 或 helper_functions。
"""
import json
...
def evaluate_rag(retriever, num_questions: int = 5) -> Dict[str, Any]:
    """
    对 RAG retriever 做评估：生成若干测试问题，对每个问题检索并打分，再基于上下文生成回答。

    Args:
        retriever: 向量检索器（如 FAISS.as_retriever()）。
        num_questions: 测试问题数量。

    Returns:
        {"questions", "results", "answers", "average_scores"}
    """
...
def _parse_eval_json(s: str) -> Dict[str, float]:
    """从评估结果字符串中解析 relevance/completeness/conciseness。"""
    s = s.strip()
    if "```" in s:
        parts = s.split("```")
        for p in parts:
